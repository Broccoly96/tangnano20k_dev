...
- `eth_udp_reg_tui.py` has `src filter`
- `eth_udp_reg_tui.py` supports `raw/decode`
- `uart_log_tool.py --transport udp` supports `decode`
- `uart_log_tool.py --transport udp` shows current `udp_stream`
...
| Event name       | `event_id` | Trigger                                           | arg usage             | Notes           |
| ---------------- | ---------: | ------------------------------------------------- | --------------------- | --------------- |
| `PCIE_EP_STATUS` |     `0x01` | status request while PCIe status source is active | aggregated PCIe state | on-demand event |
