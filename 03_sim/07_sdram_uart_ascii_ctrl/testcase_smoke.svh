  initial begin : tc_smoke
    ...
    send_text("R 100308\n");
    expect_cmd(ASCII_OP_READ, 1'b0, 21'h100308, 32'h0000_0000, 21'h0, "read");

    log_info("ASCII CTRL TB", "case: single write");
    send_text("W 100308 89ABCDEF\n");
    expect_cmd(ASCII_OP_WRITE, 1'b0, 21'h100308, 32'h89AB_CDEF, 21'h0, "write");

    log_info("ASCII CTRL TB", "case: bulk read");
    send_text("BR 100000 0040\n");
    expect_cmd(ASCII_OP_BULK, 1'b1, 21'h100000, 32'h0000_0000, 21'h00040, "bulk read");

    log_info("ASCII CTRL TB", "case: bulk write");
    send_text("BW 100000 0040\n");
    expect_cmd(ASCII_OP_BULK, 1'b0, 21'h100000, 32'h0000_0000, 21'h00040, "bulk write");

    log_info("ASCII CTRL TB", "case: optional 0x and lowercase hex");
    send_text("W 0x000012 0xdeadbeef\n");
    expect_cmd(ASCII_OP_WRITE, 1'b0, 21'h000012, 32'hDEAD_BEEF, 21'h0, "0x lower hex");

    log_info("ASCII CTRL TB", "case: repeated spaces and CRLF");
    send_text("R   000120");
    send_byte(ASCII_CMD_CR);
    send_byte(ASCII_CMD_LF);
    expect_cmd(ASCII_OP_READ, 1'b0, 21'h000120, 32'h0000_0000, 21'h0, "spaces crlf");
    ...
    send_byte(8'h14);
    send_text("R 000120\n");
    expect_cmd(ASCII_OP_READ, 1'b0, 21'h000120, 32'h0000_0000, 21'h0, "control ignore");
