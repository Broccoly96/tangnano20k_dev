...
// Description  : Minimal SSD1306 I2C command sequencer for 128x32 panels.
//                Supported operations:
//                  - init using a standard 128x32 SSD1306 setup
//                  - display on / display off
//                  - clear full frame
//                  - full-frame write of 512 bytes
//////////////////////////////////////////////////////////////////////////////////
...
module ssd1306_display_ctrl #(
  ...
  input  logic [(ssd1306_uart_proto_pkg::SSD1306_FRAME_BYTES*8)-1:0] I_REQ_FRAME_DATA,
  ...
  localparam int unsigned MAX_SEQ_BYTES     = 1 + 1 + SETUP_CMD_BYTES + 1 + FRAME_DATA_BYTES;
  localparam int unsigned STEP_W            = (MAX_SEQ_BYTES <= 1) ? 1 : $clog2(MAX_SEQ_BYTES);
  ...
  // Keeps the byte-controller request bus quiescent except for single-cycle issue pulses.
  ...
  // Serializes one complete SSD1306 request into START, byte writes, and STOP.
  always_ff @(posedge I_CLK or negedge I_RST_N) begin
    integer frame_idx;
    if (!I_RST_N) begin
      st_state            <= ST_IDLE;
      ...
      case (st_state)
          ...
          if (I_REQ_VALID) begin
            ...
            if (LATCH_REQ_FRAME_DATA) begin
              for (frame_idx = 0; frame_idx < SSD1306_FRAME_BYTES; frame_idx++) begin
                r_active_frame_data[frame_idx] <= I_REQ_FRAME_DATA[frame_idx*8 +: 8];
              end
            end
            r_step_idx          <= '0;
            r_txn_phase         <= 1'b0;
        ...
        ST_WAIT_BYTE: begin
          if (l_byte_done) begin
            ...
            end else if (r_step_idx + 1'b1 >= seq_len(r_active_op, r_txn_phase)) begin
              st_state <= ST_ISSUE_STOP;
            end else begin
        ...
        default: begin
          if (l_byte_done) begin
            if ((r_done_status == 32'h0000_0000) &&
                !r_txn_phase &&
                ((r_active_op == DISP_OP_CLEAR) || (r_active_op == DISP_OP_FRAME_WRITE))) begin
              r_txn_phase <= 1'b1;
              r_step_idx  <= '0;
              st_state    <= ST_ISSUE_START;
            end else begin
...
module ssd1306_display_stream_ctrl #(
  ...
  output logic O_REQ_READY,
  input  logic [2:0] I_REQ_OP,
  output logic [((ssd1306_uart_proto_pkg::SSD1306_FRAME_BYTES <= 1) ? 1 : $clog2(ssd1306_uart_proto_pkg::SSD1306_FRAME_BYTES))-1:0] O_FRAME_BYTE_IDX,
  ...
  assign O_BUSY           = (st_state != ST_IDLE);
  assign O_FRAME_BYTE_IDX = ((r_active_op == DISP_OP_FRAME_WRITE) && r_txn_phase && (r_step_idx >= 2)) ?
                            FRAME_BYTE_IDX_W'(r_step_idx - 2) : '0;
