...
// Purpose      : UART ASCII bridge for direct SSD1306 control and raw frame
//                upload from the host.
// Behavior     : Parses single-line ASCII commands, receives raw frame blocks
//                through `sdram_uart_bulk_rx`, buffers the full 512-byte frame,
//                and forwards display requests into `ssd1306_display_stream_ctrl`.
// Usage        : Accepted command lines are `I`, `C`, `O`, `X`, and `W`.
//                `W` starts bulk receive mode and must be followed by raw frame
//                packets until the terminating empty block is received.
//////////////////////////////////////////////////////////////////////////////////
...
module ssd1306_uart_bridge_ctrl #(
  ...
  localparam int unsigned FRAME_WORD_IDX_W =
    (FRAME_WORDS <= 1) ? 1 : $clog2(FRAME_WORDS + 1);
  localparam int unsigned FRAME_BYTE_IDX_W =
    (SSD1306_FRAME_BYTES <= 1) ? 1 : $clog2(SSD1306_FRAME_BYTES);

  // Bulk receive FSM:
  // - BULK_IDLE waits for ASCII commands.
  // - BULK_WRITE_RECV accepts raw frame packets and stores them locally.
  // - BULK_WRITE_WAIT_DONE waits for the display-stream controller to complete
  //   the frame write request.
  // Flow:
  //   BULK_IDLE -> BULK_WRITE_RECV -> BULK_WRITE_WAIT_DONE -> BULK_IDLE
  // Transition conditions:
  // - `W` command enters BULK_WRITE_RECV.
  // - A zero-length bulk block after exactly 512 bytes launches the display
  //   frame-write request and enters BULK_WRITE_WAIT_DONE.
  // - Timeout, protocol errors, aborts, and display completion return to
  //   BULK_IDLE and push an event into the host FIFO.
  ...
  logic s_disp_frame_byte_req;
  logic [FRAME_BYTE_IDX_W-1:0] s_disp_frame_byte_idx;
  logic s_disp_frame_byte_valid;
  ...
  assign s_cli_enable    =
    I_ENABLE &&
    (HOST_EVT_IF.enable || s_bulk_active || s_disp_busy || r_disp_req_valid);
  ...
  assign l_bulk_rx_word_ready =
    (st_bulk == BULK_WRITE_RECV) && (r_bulk_word_index < FRAME_WORDS);
  assign s_disp_frame_byte_valid = 1'b1;
  ...
  ssd1306_display_stream_ctrl #(
    ...
    .O_FRAME_BYTE_REQ     (s_disp_frame_byte_req),
    .O_FRAME_BYTE_IDX     (s_disp_frame_byte_idx),
    .I_FRAME_BYTE_VALID   (s_disp_frame_byte_valid),
  ...
  // Parses ASCII commands, manages raw frame upload state, stores accepted
  // bulk words into the local frame buffer, and forwards high-level requests
  // into the SSD1306 streaming display controller.
  always_ff @(posedge I_CLK or negedge I_RST_N) begin
    integer line_idx;
    ...
    if (!I_RST_N) begin
      ...
      if ((st_bulk == BULK_WRITE_RECV) &&
          (BULK_RX_TIMEOUT_CYCLES > 0) &&
          (r_bulk_timeout_cnt >= BULK_RX_TIMEOUT_CYCLES)) begin
        st_bulk <= BULK_IDLE;
        arm_event(
          EVT_FRAME_ERR,
          ERR_BULK_TIMEOUT,
          r_bulk_received_bytes,
          32'h0000_0000
        );
      end
      ...
      if (I_ENABLE &&
        ...
        if (I_CLI_RX_DATA == ASCII_CMD_CR) begin
          ...
          // Source-select control bytes are forwarded by uart_log_cli as
          // escaped literals. Ignore them so they do not poison the next
          // ASCII command line.
      ...
      if ((st_bulk == BULK_WRITE_RECV) && l_bulk_rx_block_done) begin
        ...
        if (l_bulk_rx_block_bytes == 0) begin
          ...
          if (next_total_bytes > SSD1306_FRAME_BYTES) begin
            ...
            arm_event(
              EVT_FRAME_PROG,
              pack_frame_progress_arg0(
                r_bulk_chunk_index,
                l_bulk_rx_block_bytes[7:0],
                next_total_bytes
              ),
